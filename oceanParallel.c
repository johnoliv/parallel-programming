...
int main(int argc, char * argv[]) {
    ...
    if(indEntValida){
        ...
        qtdThreads = t;
        thread_handles = malloc (qtdThreads * sizeof(pthread_t));
        ...
        while (!fim) {
            ...
            // executa as threads
            for(thr = 0; thr < qtdThreads; thr++)
                pthread_create(&thread_handles[thr], NULL, verifMatriz, (void*) thr);

            for (thr = 0; thr < qtdThreads; thr++)
                pthread_join(thread_handles[thr], NULL); 
...
void *verifMatriz(void *rank){ 
    ...
    int linhaTLocal, primLinha, ultLinha;
    int i, j;

    // divide a tarefa
    linhaTLocal = linhaMatriz/qtdThreads;
    primLinha = my_rank*linhaTLocal;
    ultLinha = (primLinha+1)+linhaTLocal-1;
    ...
    // Como minhas matrizes tem tamanho maior,
    // ajusto a posicao da matriz principal.
    primLinha++;
    ultLinha++;

    if(ultLinha > linhaMatriz){ ultLinha = linhaMatriz; } 

    for (i = primLinha; i < ultLinha; i++) {
        for (j = 1; j < colunaMatriz; j++) {  
